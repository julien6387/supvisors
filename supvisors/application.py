...
class ApplicationRules(object):
    ...
    Attributes are:
        - start_sequence: defines the order of this application when starting
        all the applications in the DEPLOYMENT state,
        0 means: no automatic start,
        - stop_sequence: defines the order of this application when stopping
        all the applications,
        0 means: immediate stop,
        - starting_failure_strategy: defines the strategy (in
        StartingFailureStrategies) to apply when a required process cannot be
        started during the starting of the application,
        - running_failure_strategy: defines the default strategy (in
        RunningFailureStrategies) to apply when a required process crashes when
...
class ApplicationStatus(object):
    ...
    Attributes are:
        - application_name: the name of the application, corresponding to the
        Supervisor group name,
        - state: the state of the application in ApplicationStates,
        - major_failure: a status telling if a required process is stopped
        while the application is running,
        - minor_failure: a status telling if an optional process has crashed
        while the application is running,
        - processes: the map (key is process name) of the ProcessStatus
        belonging to the application,
        - rules: the ApplicationRules instance applicable to the application,
        - start_sequence: the sequencing to start the processes belonging to
        the application, as a dictionary.
        The value corresponds to a list of processes having the same sequence
        order, used as key.
        - stop_sequence: the sequencing to stop the processes belonging to the
    ...
    def running(self):
        ...
        return self.state in [ApplicationStates.STARTING,
                              ApplicationStates.RUNNING]
    ...
    def state(self, newState):
        if self._state != newState:
            ...
            self.logger.info('Application {} is {}'
    ...
    def update_sequences(self):
        """ Evaluate the sequencing of the starting / stopping application
        from its list of processes. """
        # fill ordering iaw process rules
        self.start_sequence.clear()
        self.stop_sequence.clear()
        for process in self.processes.values():
            self.start_sequence.setdefault(process.rules.start_sequence,
                                           []).append(process)
            self.stop_sequence.setdefault(process.rules.stop_sequence,
                                          []).append(process)
    ...
    def update_status(self):
        ...
        for process in self.processes.values():
            ...
            if process.state == ProcessStates.RUNNING:
                running = True
            elif process.state in [ProcessStates.STARTING,
        ...
        # update major_failure and minor_failure status
