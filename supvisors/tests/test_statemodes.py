...
@pytest.fixture
def state_modes(supvisors):
    """ Create a SupvisorsStateModes with no discovery mode. """
    supvisors.options.multicast_group = None
    return SupvisorsStateModes(supvisors)


@pytest.fixture
def state_modes_discovery(supvisors):
    """ Create a SupvisorsStateModes with discovery mode. """
    supvisors.options.multicast_group = '239.0.0.1:6666'
    return SupvisorsStateModes(supvisors)


@pytest.fixture
def simple_sm(supvisors):
    """ Create a SupvisorsStateModes with no discovery mode and simplified mapper. """
    supvisors.options.multicast_group = None
    supvisors.mapper._instances = {k: v for k, v in supvisors.mapper.instances.items()
                                   if k.startswith('10.0.0.')}
    supvisors.mapper.local_identifier = '10.0.0.1:25000'
    return SupvisorsStateModes(supvisors)


def test_supvisors_state_modes(supvisors, state_modes):
    """ Test the SupvisorsStateModes creation. """
    assert state_modes.supvisors is supvisors
    assert state_modes.logger is supvisors.logger
    assert state_modes.mapper is supvisors.mapper
    assert state_modes.external_publisher is supvisors.external_publisher
    assert sorted(state_modes.instance_state_modes.keys()) == sorted(supvisors.mapper.instances.keys())
    for identifier, sm in state_modes.instance_state_modes.items():
        assert type(sm) is StateModes
        assert identifier == sm.identifier
        if identifier == supvisors.mapper.local_identifier:
    ...
    # test accessors
    assert state_modes.local_identifier == supvisors.mapper.local_identifier
    assert state_modes.local_state_modes is state_modes.instance_state_modes[supvisors.mapper.local_identifier]
...
def test_supvisors_state_modes_discovery(supvisors, state_modes_discovery):
...
def test_supvisors_state_modes_normal(supvisors, simple_sm):
    ...
    assert supvisors.rpc_handler.send_state_event.call_args_list == [call(simple_sm.local_state_modes.serial())]
    ...
    assert supvisors.rpc_handler.send_state_event.call_args_list == [call(expected)]
    supvisors.rpc_handler.send_state_event.reset_mock()
    ...
    assert supvisors.external_publisher.send_supvisors_status.call_args_list == [call(published)]
    supvisors.external_publisher.send_supvisors_status.reset_mock()

    # Trigger Master selection (no core identifiers)
    assert not supvisors.mapper.core_identifiers
...
def test_select_master_core(supvisors, simple_sm):
    """ Test Master selection when core_identifiers are involved. """
    supvisors.mapper._core_identifiers = ['10.0.0.2', '10.0.0.3']
...
def test_supvisors_state_modes_established(supvisors, simple_sm):
    """ Test the SupvisorsStateModes accessors on the local state & modes.
...
def test_supvisors_state_modes_split_brain(supvisors, simple_sm):
    """ Test the SupvisorsStateModes accessors on the local state & modes.
    Follow the expected logic of events in a case where the local Supvisors instance is started
...
def test_supvisors_state_modes_split_brain_core(supvisors, simple_sm):
    """ Same test in a Supvisors split-brain context with core_identifiers. """
    supvisors.mapper._core_identifiers = ['10.0.0.4', '10.0.0.3']
    # quickly set context
    event = {'fsm_statecode': 4, 'fsm_statename': 'OPERATION',
             'degraded_mode': True, 'discovery_mode': False,
             'master_identifier': '10.0.0.3:25000',
...
def test_supvisors_state_modes_user(supvisors, simple_sm):
    """ Test the SupvisorsStateModes accessors on the local state & modes.
