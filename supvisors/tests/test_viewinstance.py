...
def view(http_context):
    ...
    return SupvisorsInstanceView(http_context, SupvisorsPages.HOST_INSTANCE_PAGE)
...
def test_init_no_stats(view_no_stats):
    ...
    # test parameter page name
    assert view_no_stats.page_name == SupvisorsPages.HOST_INSTANCE_PAGE
...
def test_write_navigation(mocker, supvisors_instance, view):
...
def test_write_status(mocker, supvisors_instance, view):
    ...
    status = supvisors_instance.context.local_status
    sm = supvisors_instance.state_modes.local_state_modes
    ...
    assert master_mid.content.call_args_list == [call(SupvisorsSymbols.MASTER_SYMBOL)]
...
def test_write_actions(mocker, view):
    ...
    expected = [call('', SupvisorsPages.HOST_INSTANCE_PAGE, **{ACTION: 'stopall'}),
                call('', SupvisorsPages.HOST_INSTANCE_PAGE, **{ACTION: 'restartsup'}),
                call('', SupvisorsPages.HOST_INSTANCE_PAGE, **{ACTION: 'shutdownsup'})]
...
def test_restart_sup_action(mocker, view):
    ...
    cb = view.restart_sup_action()
    assert cb() == ('warn', 'Supervisor restart requested at now')
...
def test_shutdown_sup_action(mocker, view):
    ...
    local_identifier = view.supvisors.mapper.local_identifier
    cb = view.shutdown_sup_action()
    assert cb() == ('warn', 'Supervisor shutdown requested at now')
    assert mocked_pusher.call_args_list == [call(local_identifier)]
