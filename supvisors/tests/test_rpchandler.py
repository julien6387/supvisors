...
def test_all_requests(mocker, supvisors):
    ...
    assert proxy_server.post_request.call_args_list == [call(*expected)]
    proxy_server.post_request.reset_mock()
...
def test_all_publications(mocker, supvisors):
    ...
    assert proxy_server.publish.call_args_list == [call(expected)]
    proxy_server.publish.reset_mock()
    # test push PROCESS
...
def test_discovery_event(mocker, supvisors):
    ...
    handler.post_discovery(expected)
    assert proxy_server.post_discovery.call_args_list == [call(expected)]
    proxy_server.publish.reset_mock()
    # check that the other methods have not been called
    assert not proxy_server.post_request.called
    assert not proxy_server.publish.called
