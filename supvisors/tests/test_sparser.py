...
class CommonParserTest(unittest.TestCase):
    ...
    def check_valid(self, parser):
        ...
        # check single address with required not applicable and out of
        # range loading
        process = ProcessStatus('dummy_application_B', 'dummy_program_B2',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules, ['10.0.0.3'], 0, 0,
                                  False, False, 1,
                                  RunningFailureStrategies.RESTART_PROCESS)
        ...
        process = ProcessStatus('dummy_application_B', 'dummy_program_B3',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules, ['*'], 0, 0,
                                  False, False, 100,
                                  RunningFailureStrategies.STOP_APPLICATION)
        ...
        process = ProcessStatus('dummy_application_C', 'dummy_program_C3',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules, ['#'], 1, 0,
                                  True, True, 1,
                                  RunningFailureStrategies.CONTINUE)
        ...
        process = ProcessStatus('dummy_application_D', 'dummies_01_any',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules, ['#'], 1, 1,
                                  False, True, 75,
                                  RunningFailureStrategies.CONTINUE)
...
class ElementTreeParserTest(CommonParserTest):
    ...
    def check_invalid(self, parser):
        ...
        process = ProcessStatus('dummy_application_B', 'dummy_program_B1',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules, ['#'], 3, 50,
                                  True, False, 5,
                                  RunningFailureStrategies.CONTINUE)
        # check single address with required not applicable and out of
        # range loading
        ...
        self.assert_process_rules(process.rules,
                                  ...
                                  RunningFailureStrategies.RESTART_APPLICATION)
        # check multiple addresses, all other incorrect values
        process = ProcessStatus('dummy_application_B', 'dummy_program_B5',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules,
                                  ['10.0.0.3', '10.0.0.1', '10.0.0.5'],
                                  0, 0, False, False, 1,
                                  RunningFailureStrategies.CONTINUE)
        # check empty reference
        process = ProcessStatus('dummy_application_C', 'dummy_program_C0',
        ...
        process = ProcessStatus('dummy_application_C', 'dummy_program_C4',
                                self.supvisors)
        parser.load_process_rules(process)
        self.assert_process_rules(process.rules,
                                  ['10.0.0.4', '10.0.0.2'],
                                  0, 100, False, False, 10,
                                  RunningFailureStrategies.CONTINUE)
        # check pattern with single matching and reference
