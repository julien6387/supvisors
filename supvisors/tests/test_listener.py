...
class ListenerTest(unittest.TestCase):
    ...
    def test_on_process(self, mocked_time):
        ...
        self.assertEqual([call({'processname': 'dummy_process', 'groupname': 'dummy_group',
            'state': 200, 'now': 77, 'pid': 1234, 'expected': True})], listener.publisher.send_process_event.call_args_list)

    @patch('supvisors.listener.instant_statistics', return_value=(8.5, [(25, 400)], 76.1, {'lo': (500, 500)}, {}))
    def test_on_tick(self, mocked_stats):
        """ Test the reception of a Supervisor TICK event. """
        from supvisors.listener import SupervisorListener
        listener = SupervisorListener(self.supvisors)
        # create patches
        listener.publisher = Mock(**{'send_tick_event.return_value': None, 'send_statistics.return_value': None})
        ...
        self.assertEqual([call({'when': 120})], listener.publisher.send_tick_event.call_args_list)
        self.assertEqual([call((8.5, [(25, 400)], 76.1, {'lo': (500, 500)}, {}))], listener.publisher.send_statistics.call_args_list)
        self.assertEqual([call()], listener.fsm.on_timer_event.call_args_list)
        self.assertEqual([call(['10.0.0.1', '10.0.0.4'])], self.supvisors.zmq.pusher.send_isolate_addresses.call_args_list)
    ...
    def test_unstack_event(self):
        ...
        self.assertEqual([call('10.0.0.1', 'data')], listener.fsm.on_tick_event.call_args_list)
        ...
        self.assertEqual([call('10.0.0.2', {"name": "dummy"})], listener.fsm.on_process_event.call_args_list)
        self.assertFalse(listener.statistician.push_statistics.called)
        ...
        self.assertFalse(listener.fsm.on_tick_event.called)
        self.assertFalse(listener.fsm.on_process_event.called)
        self.assertEqual([call('10.0.0.3', [0, [[20, 30]], {"lo": [100, 200]}, {}])], listener.statistician.push_statistics.call_args_list)
    ...
    def test_unstack_info(self):
        ...
        self.assertEqual([call('10.0.0.4', {"name": "dummy"})], listener.fsm.on_process_info.call_args_list)
    ...
    def test_on_remote_event(self):
        ...
        with patch.multiple(listener, unstack_event=DEFAULT, unstack_info=DEFAULT, authorization=DEFAULT):
            # test unknown type
            ...
            listener.on_remote_event(event)
            self.assertEqual([call({'state': 'RUNNING'})], listener.unstack_event.call_args_list)
            self.assertFalse(listener.unstack_info.called)
            self.assertFalse(listener.authorization.called)
            ...
            listener.on_remote_event(event)
            self.assertFalse(listener.unstack_event.called)
            self.assertEqual([call({'name': 'dummy_process'})], listener.unstack_info.call_args_list)
            self.assertFalse(listener.authorization.called)
            ...
            self.assertFalse(listener.unstack_event.called)
            self.assertFalse(listener.unstack_info.called)
            self.assertEqual([call(('10.0.0.1', True))], listener.authorization.call_args_list)
    ...
    def test_force_process_state(self, mocked_time):
        ...
        self.assertEqual([call({'processname': 'process', 'groupname': 'appli', 'state': 200,
            'now': 56, 'pid': 0, 'expected': False})], listener.publisher.send_process_event.call_args_list)
