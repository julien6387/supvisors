...
from supervisors.rpcrequests import getRPCInterface
from supervisors.utils import SUPERVISORS_EVENT, SUPERVISORS_INFO, SUPERVISORS_TASK
...
def set_async_proxy(*args):
    ...
    SupervisorsPool.proxy = getRPCInterface("localhost", args[0])
...
class SupervisorsPool:
    ...
    Supervisors works in the context of the main thread of the supervisor daemon.
    It consequently blocks any incoming XML-RPC as long as its job is in progress.
    The problem is that Supervisors sometimes uses XML-RPC towards another supervisor daemon running elsewhere.
    If the Supervisors of the other instance is doing the same at the same time, both are blocking themselves.

    That's why the XML-RPC performed by Supervisors are performed asynchronously when possible.
    ...
    def __init__(self, supervisors):
        ...
        self.env = supervisors.info_source.get_env()
    ...
    def async_event(self):
        ...
        self.pool.apply_async(async_remote_comm_event, (SUPERVISORS_EVENT, ))

    def async_task(self):
        """ Use an asynchronous remote communication event to inform
        that a periodic task can be performed. """
        self.pool.apply_async(async_remote_comm_event, (SUPERVISORS_TASK, ))
